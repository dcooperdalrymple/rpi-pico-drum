...
# usage: python3 img_to_array.py <image.bmp>
...
if INVERT:
    pixels = [1 if x == 255 else 0 for x in pixels]
else:
...
buffer = ", ".join(buffer)
buffer_hex = f'static uint8_t {img_name}_img[] = {{{buffer}}};\n'

with open(f'{img_name}_img.h', 'wt') as file:
    file.write(f'#define {img_name.upper()}_IMG_WIDTH {img_width}\n')
    file.write(f'#define {img_name.upper()}_IMG_HEIGHT {img_height}\n\n')
